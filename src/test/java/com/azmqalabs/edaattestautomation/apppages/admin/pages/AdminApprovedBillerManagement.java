	...
	public void VerifyTransferRemarkFieldIsChecked(Map<Object, Object> testdatamap, Log Log) throws Exception {
		try {
			...
			if (IsSelected(EdaatOR.Admin_SystemMgm_TransferRemarkCheckbox)) {
				test.log(Status.PASS, " Transfer Remark field is Checked" + driver.getTitle()
						+ "Transfer Remark field is Checked PASS * ");
				Log.ReportEvent("PASS", "Transfer Remark field is Checked");
				this.takeScreenShot();
			} else {
				test.log(Status.FAIL, " Transfer Remark field is NotChecked" + driver.getTitle()
						+ " * Transfer Remark field is NotChecked FAIL ");
				this.takeScreenShot();
			}
		} catch (Exception e) {
			e.printStackTrace();
